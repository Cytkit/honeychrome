...
def compute_mfis(controller, state, all_rel, n_clusters,
                 ...
                 names_override: dict | None = None) -> pd.DataFrame | None:
    """
    ...
    sample_vals = {}
    for rel in all_rel:
        mv = drc_pipeline.load_sample_marker_values(controller, state, rel)
        if mv is not None:
...
def run_statistics(controller, state, run_freq: bool, run_mfi: bool,
    ...
    returns ``(freq_results, mfi_results)`` (either may be None).
    ...
    return freq_results, mfi_results
